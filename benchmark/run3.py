...
def rb_contains(bitmap_bytes: bytes, entity_id: int) -> bool:
    """Check if entity_id is in bitmap. Caches deserialized bitmap by identity."""
    key = id(bitmap_bytes)
...
def load_one_conv(con: duckdb.DuckDBPyConnection, name: str,
                  conv_root: Path, facts_root: Path = None):
    """Load a single convolution or facts table into DuckDB TABLE."""
    if name == "facts":
        if facts_root and facts_root.exists():
            ...
            size_mb = sum(f.stat().st_size for f in facts_root.rglob("*.parquet")) / 1e6
            print(f"  Loaded facts: TABLE ({size_mb:.1f}MB)")
        return

    glob_path = str(conv_root / "**" / f"{name}.parquet")
    files     = list(conv_root.rglob(f"{name}.parquet"))
    if not files:
    ...
    print(f"  Loaded {name}: TABLE ({size_mb:.1f}MB)")


def unload_one_conv(con: duckdb.DuckDBPyConnection, name: str):
    """Drop a convolution table to free memory."""
    try:
    ...
    for name in sorted(output_names):
        ...
        try:
            if can_load_memory:
                ...
                load_type = f"TABLE ({size_mb:.1f}MB in memory)"
            else:
                con.execute(
...
def run_benchmark(benchmark_id: str, entity: str, data_dir: str = "./data",
    ...
    current_conv = None  # track what's currently loaded
    ...
    for q in spec["queries"]:
        ...
        # lazy load per query — hardcoded for B1
        if benchmark_id == "B1":
            needed = {
                "Q0": "conv_cab_type",
                "Q1": "conv_cab_type",
                "Q2": "facts",
                "Q3": "conv_pax_distance_date",
                "Q4": "conv_pax_distance_date",
            }.get(qid)
            if needed and needed != current_conv:
                if current_conv:
                    unload_one_conv(con, current_conv)
                load_one_conv(con, needed, conv_root, facts_root)
                current_conv = needed
